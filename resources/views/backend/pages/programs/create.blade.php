            ...
            <div class="col-xl-10 col-lg-10 col-md-12 col-sm-12 col-10 col-xs-12">
                <div class="row">
                    ...
                    <div class="col-12 mt-5">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="header-title">@if(isset($program)) Edit Program @else Programs @endif </h4>
                                <x-form has-files :action="isset($program) ? route('programs.update', $program->id): route('programs.store')" :method="isset($program) ? 'PUT': 'POST'">
                                    <div class="form-group">
                                        <x-input.label for="title">Title</x-input.label>
                                            <x-input.text type="text" class="form-control @error('title') error-message @enderror" id="title" :value="isset($program) ? old('title',$program): old('title')" name="title" placeholder="Title"
                                                required />
                                            @error('title')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <x-input.text type="text" class="form-control @error('location') error-message @enderror" id="location" :value="isset($program) ? old('location',$program):old('location')" name="location" placeholder="Location"   required />
                                        @error('location')
                                      ...
                                      <div class="form-group">
                                        <x-input.label for="description">Description</x-input.label>
                                            <textarea id="description" class="form-control @error('description') error-message @enderror" name="description" aria-label="With textarea">{{ isset($program) ? old('description',$program):old('description')}}</textarea>
                                            @error('description')
                                                <x-input.error id="" class="form-text text-danger">{{ $message }}</x-input.error>
                                            @enderror
                                    </div>
                                    <div class="mb-3">
                                        ...
                                        <div class="media mb-2 mt-2">
                                            <img class="img-fluid mr-4" src="{{Storage::url($program->photo)}}" style="height: 100px;" alt="image">
